...
.panel-right { width: 320px; min-width: 320px; border-right: none; border-left: 1px solid #ddd; }
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Delete' || e.key === 'Backspace') {
        ...
        e.preventDefault();
        if (selIdx > 0) selectAnn(selIdx - 1);
    } else if (e.key === 'ArrowDown') {
        e.preventDefault();
        if (selIdx < anns.length - 1) selectAnn(selIdx + 1);
    }
});
